...
- This may also solve the `LZCompressLib.dll` redistribution question. The distribution of Binary probably ships that DLL, because Binary depends on Nikki. If so, we can load it from the Binary install instead of shipping our own copy. Confirm this against a real 2.8.3 install before we rely on it.
...
3. **What a real Binary 2.8.3 install holds.** The prerequisite section assumes a layout. Confirm it against a real install. Check the exact path of `mainkeys/` and `userkeys/`. Check the file name for each supported game. Find out how to read the version, so that step 3 of the validation can work. Check whether the install also ships `LZCompressLib.dll`, which would remove the next question.
4. **License confirmation for `LZCompressLib.dll`.** It is a closed-source native blob inside an MIT repo, with no separate license text and no public source. The MIT grant of Nikki almost certainly covers it. This matters only if we ship our own copy. If we load the DLL from the Binary install instead, the question disappears. Otherwise one email to MaxHwoy settles it.
