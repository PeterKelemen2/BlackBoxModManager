...
5. **`GameCatalog` holds one game.** Underground 2 is the only install that we listed file by file, so it is the only entry. Step 7 adds the other five. Add an entry only after a listing of a real install confirms the executable name and the markers.
