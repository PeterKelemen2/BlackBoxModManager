...
11. **The verify's third category needs every container that a script writes, not only the ones that a manifest declares.** `ContainerDeployEngine` built the reported container list from the manifest's `Files` field alone. The Recompiled Vinyls mod writes about 49 car containers that its manifest never names. The verify then saw each one as an unexplained change and failed the deploy. `CommandGate` already found every one of these paths, because `Prepare` needs them to make each container private. The fix builds the reported list from that same set. See defect 16.
